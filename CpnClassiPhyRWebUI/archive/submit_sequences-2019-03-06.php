...
<?php
    if (isset($_REQUEST['genbank_cpn60ut_accession'])){
        ...
        $host_latin_name = stripslashes($_REQUEST['host_latin_name']);
        $host_latin_name = pg_escape_string($con,$host_latin_name);
        
        $host_common_name = stripslashes($_REQUEST['host_common_name']);
        $host_common_name = pg_escape_string($con,$host_common_name);
...
//        $_SESSION['email'] = $email;
    ...
    //    $myfile = fopen($filepath, "w") or die("Unable to open file: $filepath!");
        ...
        $genbank_16s_fasta = $current_output_dir . '/' . $genbank_16s_fasta . ".fasta";

        $curlCommand = '/usr/bin/curl --url "https://www.ncbi.nlm.nih.gov/sviewer/viewer.fcgi?tool=portal&sendto=on&log$=seqview&db=nuccore&dopt=fasta&val=' . $genbank_16s_accession . '&extrafeat=0&maxplex=1" -o ' . $genbank_16s_fasta;
        //echo $curlCommand;
        $output = shell_exec($curlCommand);
        $f = read_fas_file($genbank_16s_fasta);
        
        if(preg_match("/Failed to understand id: $genbank_16s_accession/", $f)){
            die("$genbank_16s_accession does not exist! Please check the 16S Genbank Accession ID and submit again.");
        }
        ...
        if(preg_match("/Failed to understand id: $genbank_cpn60ut_accession/", $f)){
            die("$genbank_cpn60ut_accession does not exist! Please check the Cpn60 UT Genbank Accession ID and submit again.");
        }else{
...
//                $rows = pg_num_rows($submission_result);
    ...
    $outgroup_sequence = $_SERVER['DOCUMENT_ROOT'] . '/' . 'CpnClassiPhyR/db/acholeplasma_laidlawii_PG8R10_outgroup.fasta';
    $tree_output_dir = $current_output_dir . '/' . 'phylogenetics';
    
    
    $megaTreeCommand = escapeshellcmd('/usr/bin/python3.5' . ' ' . $_SERVER['DOCUMENT_ROOT'] . '/' . "CpnClassiPhyR/generate_subgroups_tree.py -i $subgroups_fasta_infile -g $outgroup_sequence  -o $tree_output_dir");
    
    //echo $megaTreeCommand;
    $output = shell_exec($megaTreeCommand);
    
    $tree_php_dir = 'submissions_dir/output_dir' . '_' . $long . '_' . $time . '/phylogenetics';
    
    $phyloxml_tree = 'subgroups_mega_tree.xml';
    $PhyD3_tree_php_outfile = $tree_output_dir . '/subgroups_PhyD3_tree.php';
    ...
    $subgroups_tree_infile = $_SERVER['DOCUMENT_ROOT'] . '/data/xml/subgroups_mega_tree.xml';
    $subgroup_tree_outfile = $_SERVER['DOCUMENT_ROOT'] . '/data/xml/subgroups_mega_tree' . '_' . $long . '_' . $time . '.xml';
                
    $subgroups_xml_tree_file = $subgroup_metadata_src_dir . '/phylogenetics/subgroups_mega_tree.xml';
                
                
    $cpPhyloTreeCommand = "cp -rf $tree_php_dir $subgroup_metadata_src_dir && cp $subgroups_tree_infile $subgroup_tree_outfile && cp $subgroups_xml_tree_file $subgroups_tree_infile";
    $output = shell_exec($cpPhyloTreeCommand);
                ...
                if(isset($subgroup_cpn60ut_accessions[$genbank_cpn60ut_accession])){
                    ...
                    //VALUES ('$email', '$species', '$strain', '$proposed_group_subgroup', '$genbank_cpn60ut_accession', '$group_16s_subgroup', '$genbank_16s_accession', '$host_latin_name', '$country', '$reference', '$trn_date')";
                    ...
                    $new_subgroup["host-Latin name"] = $host_latin_name;
                    $new_subgroup["host-common name"] = $host_common_name;
...
<label>Host Latin Name (plant or insect)<font color="red">*</font></label>
<input type="text" class="form-control" name="host_latin_name" required />

<label>Host Common Name (plant or insect)<font color="red">*</font></label>
<input type="text" class="form-control" name="host_common_name" required />
